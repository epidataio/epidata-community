...
export EPIDATA_STREAM_PROCESSOR="$EPIDATA_HOME/spark/target/scala-2.12/epidata-stream-processor-assembly-1.0-SNAPSHOT.jar"

echo $PYTHONPATH

ps -ef | grep epidata-spark-assembly | grep -v grep | awk '{print $2}' | xargs kill

(java -jar "$EPIDATA_STREAM_PROCESSOR") & jupyter notebook --config config.py
